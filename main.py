...
def aliases(name):
    ...
    extras={"masataka yoshida":["吉田正尚"],"pedro martinez":["Pedro Martinez"],"manny ramirez":["Manny Ramirez"],"ranger suarez":["Ranger Suarez"],"jovani moran":["Jovani Moran"],"tsung-che cheng":["Tsung Che Cheng"]}
...
def q_for(p):
    ...
    return f'{np} ("Red Sox" OR "Boston Red Sox" OR MLB)'
def parse_dt(s):
...
def generate(arr):
    ...
    for p in CFG["people"]:
        ...
        (PEOPLE/f'{slug(p["name"])}.xml').write_text(rss(sub,f'{p["name"]} — Red Sox News',f'Noticias sobre {p["name"]}.'),encoding="utf-8")
    ...
    (DOCS/"index.html").write_text(f"<!doctype html><html><meta charset='utf-8'><body><h1>Red Sox News</h1><p><a href='feed.xml'>RSS general</a></p>{cards}</body></html>",encoding="utf-8")
def load_state():
    if not STATE.exists():
        return {"next_batch":1}
    try:
...
def select_batch():
    ...
    print(f"Red Sox batch {batch}: {len(selected)} names")
...
def main():
    ...
    print("Next Red Sox batch:",state["next_batch"])
